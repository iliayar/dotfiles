...
--    , position    =  Static {xpos = 0, ypos = 1060, width = 1364, height = 20}
   ...
   , template = "<action=~/.xmonad/xmonadctl 1>" ++ (icon "edved") ++ "</action> | %UnsafeStdinReader% }{ %disku% | %bright% | %default:Master% | %kbd% | %memory% | %multicpu% | %dynnetwork% | %battery% | %multicoretemp% | %date% "
