local nixcfg = require("config.nix")
...
if nixcfg.lsp.enable then
    ...
    if nixcfg.langZig.enable then
        vim.lsp.config("zls", lsp_default_config)
        vim.lsp.enable("zls")
    end

    if nixcfg.langTypescript.enable then
        vim.lsp.config("ts_ls", lsp_default_config)
        vim.lsp.enable("ts_ls")
    end

    if nixcfg.langNix.enable then
        vim.lsp.config("nixd", lsp_default_config)
    end

    if nixcfg.langLua.enable then
        vim.lsp.config("lua_ls", lsp_default_config)
        vim.lsp.enable("lua_ls")
    end

    if nixcfg.langGo.enable then
        vim.lsp.config("gopls", lsp_default_config)
        vim.lsp.enable("gopls")
    end

    if nixcfg.langPython.enable then
        vim.lsp.config("pyright", lsp_default_config)
    end

    if nixcfg.langOcaml.enable then
        vim.lsp.config("ocamllsp", lsp_default_config)
    end
    ...
    if nixcfg.langTypst.enable then
        vim.lsp.config("tinymist", lsp_default_config)
    end

    if nixcfg.langHaskell.enable then
        vim.lsp.config("hls", lsp_default_config)
    end

    if nixcfg.langLean.enable then
        vim.lsp.config("leanls", lsp_default_config)
        require("lean").setup({
            mappings = true,
            abbreviations = {
                enable = true,
    ...
    if nixcfg.langFSharp.enable then
        vim.lsp.config("fsautocomplete", lsp_default_config)
    end

    if nixcfg.langJava.enable then
        vim.lsp.config("jdtls", lsp_default_config)
    end
